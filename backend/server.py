from fastapi import FastAPI, APIRouter, HTTPException, Form, UploadFile, File, Query
...
async def generate_research_memo(request: MemoGenerationRequest):
    ...
    try:
        ...
        # Generate memo with optimized timeout handling
        try:
            memo_result = await asyncio.wait_for(
                generator.generate_research_memo(
                    memo_data=request.memo_data,
                    memo_type=request.memo_type,
                    format_style=request.format_style
                ), timeout=12.0
            )
            logger.info(f"✅ Memo generated successfully")
        except asyncio.TimeoutError:
            logger.warning("⚠️ Memo generation timed out, creating basic memo")
            # Generate basic memo as fallback
            memo_result = await generator.generate_basic_memo(
                memo_data=request.memo_data,
                memo_type=request.memo_type
            )
...
async def structure_legal_arguments(request: LegalArgumentRequest):
    ...
    try:
        ...
        # Structure arguments with optimized timeout handling
        try:
            argument_list = await asyncio.wait_for(
                structurer.structure_legal_arguments(
            ...
            logger.info(f"✅ Arguments structured successfully")
        except asyncio.TimeoutError:
            logger.warning("⚠️ Argument structuring timed out, creating basic structure")
            # Generate basic argument structure as fallback
            argument_list = await structurer.create_basic_structure(
                argument_data=request.argument_data
            )
...
async def multi_jurisdiction_search(request: MultiJurisdictionRequest):
    ...
    try:
        ...
        # Execute search across jurisdictions with optimized timeout
        try:
            results = await asyncio.wait_for(
                searcher.search_across_jurisdictions(
                    query=request.query,
            ...
            logger.info(f"✅ Multi-jurisdiction search completed")
        except asyncio.TimeoutError:
            logger.warning("⚠️ Multi-jurisdiction search timed out, creating basic results")
            # Generate basic results as fallback
            results = await searcher.create_basic_results(
                query=request.query,
                jurisdictions=request.jurisdictions
            )
